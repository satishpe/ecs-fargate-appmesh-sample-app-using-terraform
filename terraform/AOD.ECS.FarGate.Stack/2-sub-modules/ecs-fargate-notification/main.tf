...
data "template_file" "service_tmp" {
  ...
  vars      = { 
    ...
    //== "none" ? (var.virtual_gateway != "none" ? "" : aws_ecr_repository.ecr_repo[0].repository_url) : var.app_image
    ...
    app_name          = var.app_name
    env               = var.env
    logconfig              = data.template_file.logconfig.rendered
    ssm_splunk_url = var.ssm_splunk_url
    ssm_splunk_token = var.ssm_splunk_token
...
resource "aws_ecs_service" "main" {
  ...
  network_configuration {
    ...
    subnets = [ "subnet-03315460d51f84504","subnet-04b90d3c7c17405f1" ]
